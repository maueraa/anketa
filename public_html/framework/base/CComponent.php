...
class CComponent
{
	public $lang = array('lat'=>'lotin','uz'=>'kirill','ru'=>'РУС', 'en'=>'ИНГЛИЗ');
	private $_e;
	private $_m;
	public $footer;
	 ...
	 * @see __set
	 */
	public function saveStrings($paramStr,$pkey='name'){
		$dirs = new DirectoryIterator(Yii::app()->messages->basePath);
		foreach ($dirs as $dir)
			if ($dir->isDir() && !$dir->isDot())
			{
				$translations[$dir->getFilename()] = $dir->getPathname();
			}
		if (isset($translations["uz"])) unset($translations["uz"]);
		if (isset($translations["lat"])) unset($translations["lat"]);
		foreach($translations as $trkey=>$trval)
		{
			if (file_exists($trval."/strings.php")) $strings = include($trval."/strings.php"); else $strings=array();
			$paramStr["$pkey$trkey"]=isset($paramStr["$pkey$trkey"])?$paramStr["$pkey$trkey"]:"";
			if (isset($paramStr[$pkey])) $strings[$paramStr[$pkey]]=$paramStr["$pkey$trkey"];
			if (trim($paramStr["$pkey"])=="") unset ($strings[$paramStr["$pkey"]]);
			ksort($strings);
			/**
			echo "<pre>";
			print_r($paramStr["$pkey"]);
			exit;
			/**/
			$str='<?php
return array(
';
			foreach($strings as $key=>$value){
				$str .= "\t\"".addslashes($key)."\"=>\"". str_replace("\'","'", addslashes($value))."\",\r\n";
			}
			$str.=");\r\n?>\r\n";
			file_put_contents($trval."/strings.php",$str);
			unset($str);
			unset($strings);
		}
	}
