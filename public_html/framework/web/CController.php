...
class CController extends CBaseController
{
	...
	public function createCAction($params, $controller=null){
		if ($params["controller"]!==null)
		{
			$controller = include(dirname(__FILE__)."/templates/controller.php");
			file_put_contents(Yii::app()->basePath."/controllers/".(ucfirst($params["controller"]))."Controller.php",$controller);

			$index = include(dirname(__FILE__)."/templates/index.php");
			$indexuz = include(dirname(__FILE__)."/templates/indexuz.php");
			//print_r($index);print_r($params); exit;
			$warn  = include(dirname(__FILE__)."/templates/warning.php");

			$viewpath = Yii::app()->basePath."/views/".$params["controller"];

			if (!is_dir($viewpath)) mkdir($viewpath);
			file_put_contents($viewpath."/index.php",$index);
			file_put_contents($viewpath."/indexuz.php",$indexuz);
			file_put_contents($viewpath."/warning.php",$warn);
		}
		else
		{
			$viewpath = Yii::app()->basePath."/views/".$controller->controller."/topics";
			if (!is_dir($viewpath)) mkdir($viewpath);
			$topic = include(dirname(__FILE__)."/templates/articles.php");
			$topicuz = include(dirname(__FILE__)."/templates/articlesuz.php");
			$topicru = include(dirname(__FILE__)."/templates/articlesru.php");
			$warn    = include(dirname(__FILE__)."/templates/warning.php");
			file_put_contents($viewpath."/".$params["query"].".php",$topic);
			file_put_contents($viewpath."/".$params["query"]."uz.php",$topicuz);
			file_put_contents($viewpath."/".$params["query"]."ru.php",$topicru);
			file_put_contents($viewpath."/warning.php",$warn);
		}

	}
	public function createQuery($cntName,$topicname,$cnt=0){
		$viewpath = Yii::app()->basePath."/views/$cntName/topics";
		if (!is_dir($viewpath)) mkdir($viewpath);
		$topic = include(dirname(__FILE__)."/templates/".(($cnt==0)?"articles.php":"contents.php"));
		file_put_contents($viewpath."/$topicname.php",$topic);
	}
