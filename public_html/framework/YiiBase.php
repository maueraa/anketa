...
class YiiBase
{
	private  static  $device="computer";
	public static function CyrLat($str, $dir="ctl")
	{
		if ($dir==="ltc") {
			$str=self::OGTrans($str);
			$belgi = include(dirname(__FILE__).'/transcl/ltc.php');
		}
		elseif  ($dir==="ctl") $belgi = include(dirname(__FILE__).'/transcl/ctl.php');
		else return $str;
		foreach ($belgi as $key=>$value)
		{
	...
	public static function OGTrans($str, $tutuq=null){
		$str = str_replace("`","’",$str);
		$str = str_replace("'","’",$str);
		$str = str_replace("‘","’",$str);
		$str = str_replace("’","’",$str);
		...
		$str = str_replace("G’","G‘",$str);
		if ($tutuq!==null) $str = str_replace("‘", $tutuq, $str);
		return $str;
	}
	/**
	 ...
	 */
	public static function powered($str=false)
	{
		$r=($str)?$str:(Yii::t('zii',"Copyright")." &copy; ".((date('Y')==2018)?2018:"2018-".date('Y')).", <a href='http://www.phpworld.uz'>PHPWorld.uz</a>". ", " . Yii::t('zii',"All Rights Reserved"));
		return $r;
	}

	public static function abbr($str){
		if (strtolower(substr($str,0,2))=="sh" or strtolower(substr($str,0,2))=="ch") return substr($str,0,2);
		else return mb_substr($str,0,1,"utf-8");
	}
	...
	public static function t($category,$message,$params=array(),$source=null,$language=null)
	{
		...
		if(self::$_app!==null)
		{
			...
			if(($source=self::$_app->getComponent($source))!==null)
				$message=($language==='lat')?self::CyrLat($source->translate($category,$message,"uz")):$source->translate($category,$message,$language);
		}
		if($params===array())
			return $message;
		if(!is_array($params))
