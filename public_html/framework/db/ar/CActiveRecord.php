...
abstract class CActiveRecord extends CModel
{
	...
	public function getCtrlName($str){
		$array = array(
			'a','b','c','d','e','f','g','h','i','j','k','l','m','n','o','p','q','r','s','t','u','v','w','x','y','z',
			'1','2','3','4','5','6','7','8','9','0',
		);
		$newstr="";
		$str = strtolower(Yii::CyrLat($str));
		for($i=0; $i<strlen($str); $i++)
		{
			if (in_array($str[$i],$array)) $newstr .= $str[$i];
		}
		if ($newstr=="") $newstr=date("YmdHis");
		return $newstr;
	}
	public function getList($model, $fields=array('name'), $delimeter="", $filter="true")
	{
		$r = $model->findAll($filter);
		$text="";
		$k=array();
		foreach($fields as $value)
		{
			if (is_array($value) and isset($value['text']))
				$text = $value['text'];
			if (!is_array($value)) $s[$value] = CHtml::listData($r,"id",$value);
		}
		foreach ($s[$fields[0]] as $key=>$value)
		{
			foreach($fields as $kk=>$name)
			{
				if (!is_array($name))
					if ($kk==0)	$k[$key]=$s[$name][$key];
					else $k[$key].=$delimeter.$s[$name][$key];
			}
		}
		foreach($k as $key=>$value)
		{
			$k[$key]=Yii::t("strings",$value.$text);
		}
		return $k;
	}
	/**
