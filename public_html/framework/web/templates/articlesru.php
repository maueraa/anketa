...
\$language=(Yii::app()->getLanguage()=='lat')?\"uz\":Yii::app()->getLanguage();
\$warning = include(dirname(__FILE__).'/warning.php');
if (Yii::app()->getLanguage()!=='ru')
	if (file_exists(dirname(__FILE__).'/$params[query]'.\$language.'.php'))
		\$this->redirect('/$controller->controller/articles?topic=$params[query]'.\$language);
	else
