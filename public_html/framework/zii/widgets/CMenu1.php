...
class CMenu extends CWidget
{
	 ...
	 */
	protected function renderMenuRecursive($items, $fcss=null, $lcss=null, $css=null)
	{
		$count=0;
		$hasactive = false;
		$n=count($items);
		foreach ($items as $active)
		{
			if ($active['active']) $hasactive=true;
		}
		if (!$hasactive) $items[0]["active"]=true;
		foreach($items as $item)
		{
			...
			{
				$item['linkOptions']=isset($item['linkOptions'])?$item['linkOptions']:array('class'=>$this->activeCssClass);
				$class[]=$this->activeCssClass;
			}
			if($count===1 && $this->firstItemCssClass!==null)
				$class[]=($fcss!==null)?$fcss:$this->firstItemCssClass;
			if($count===$n && $this->lastItemCssClass!==null)
				$class[]=($lcss!==null)?$lcss:$this->lastItemCssClass;
			if($this->itemCssClass!==null)
				$class[]=($css!==null)?$css:$this->itemCssClass;
			if($class!==array())
			{
			...
			{
				$item['firstItemCssClass']=isset($item['firstItemCssClass'])?$item['firstItemCssClass']:null;
				$item['lastItemCssClass']=isset($item['lastItemCssClass'])?$item['lastItemCssClass']:null;
				$item['itemCssClass']=isset($item['itemCssClass'])?$item['itemCssClass']:null;
				echo "\n".CHtml::openTag('ul',isset($item['submenuOptions']) ? $item['submenuOptions'] : $this->submenuHtmlOptions)."\n";
				$this->renderMenuRecursive($item['items'], $item['firstItemCssClass'],$item['lastItemCssClass'],$item['itemCssClass']);
				echo CHtml::closeTag('ul')."\n";
			}
